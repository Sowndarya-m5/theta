...
class InfDetails extends StatefulWidget {
  final InfDataModel infDataModel;
  const InfDetails({Key? key, required this.infDataModel}) : super(key: key);

  @override
  State<InfDetails> createState() => _InfDetailsState();
}

class _InfDetailsState extends State<InfDetails> {
  ...
  Widget build(BuildContext context) {
    ...
    DateTime eventDateTime = DateFormat(
      "dd.MM.yyyy hh:mm a",
    ).parse("${widget.infDataModel.date} ${widget.infDataModel.time}");
        ...
        child: Column(
          ...
          children: <Widget>[
            Padding(
              ...
              child: Image.asset(widget.infDataModel.ImageUrl,),
            ),
            SizedBox(height: 18),
            Padding(
                ...
                child: Column(
                  children: [
                    if (now.isBefore(registrationDeadline))
                      ElevatedButton(
                        onPressed: () async {
                          String googleFormUrl = widget.infDataModel.reglink;
