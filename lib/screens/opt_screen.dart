...
class OptDetails extends StatefulWidget {
  final OptDataModel optDataModel;
  const OptDetails({Key? key, required this.optDataModel}) : super(key: key);

  @override
  State<OptDetails> createState() => _OptDetailsState();
}

class _OptDetailsState extends State<OptDetails> {
  ...
  Widget build(BuildContext context) {
    ...
    DateTime eventDateTime = DateFormat(
      "dd.MM.yyyy hh:mm a",
    ).parse("${widget.optDataModel.date} ${widget.optDataModel.time}");
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Padding(
              ...
              child: Image.asset(widget.optDataModel.ImageUrl,),
            ),
            SizedBox(height: 18),
            Padding(
                ...
                child: Column(
                  children: [
                    if (now.isBefore(registrationDeadline))
                      ElevatedButton(
                        onPressed: () async {
                          String googleFormUrl = widget.optDataModel.reglink;
