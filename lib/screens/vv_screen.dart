...
class vvDetails extends StatefulWidget {
  final vvDataModel vvdateModel;
  const vvDetails({Key? key, required this.vvdateModel}) : super(key: key);

  @override
  State<vvDetails> createState() => _vvDetailsState();
}

class _vvDetailsState extends State<vvDetails> {
  ...
  Widget build(BuildContext context) {
    ...
    DateTime eventDateTime = DateFormat(
      "dd.MM.yyyy hh:mm a",
    ).parse("${widget.vvdateModel.date} ${widget.vvdateModel.time}");
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Padding(
              ...
              child: Image.asset(widget.vvdateModel.ImageUrl,),
            ),
            SizedBox(height: 18),
            Padding(
              padding: EdgeInsets.all(8.0),
                ...
                child: Column(
                  children: [
                    if (now.isBefore(registrationDeadline))
                      ElevatedButton(
                        onPressed: () async {
                          String googleFormUrl = widget.vvdateModel.reglink;
