...
import '../screens/ele_screen.dart';
import '../services/utils.dart';
import '../wigets/text_widget.dart';

// ignore: must_be_immutable
class EleInScreen extends StatelessWidget {
  EleInScreen({Key? key}) : super(key: key);
  ...
  final List<ElecDataModel> Ele = List.generate(
      Name.length,
      (index) => ElecDataModel('${Name[index]}', '${url[index]}',
  ...
  Widget build(BuildContext context) {
    ...
    List<ElecDataModel> events = Ele.where((item) => item.cname.contains("Events") && isEventUpcoming(item.date, item.time)).toList();
    List<ElecDataModel> workshops = Ele.where((item) => item.cname.contains("Workshop") && isEventUpcoming(item.date, item.time)).toList();
  ...
  Widget _buildGridView(List<ElecDataModel> data, double screenWidth, String emptyMessage) {
    if (data.isEmpty) {
      return Center(
        child: Text(
          emptyMessage,
        ...
        return InkWell(
          onTap: () {
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (context) => EleDetails(eleDataModel: data[index]),
              ),
            );
          },
          child: Container(
